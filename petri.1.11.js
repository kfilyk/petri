...
var netAge=0; //Total number of years all petris have lived (recorded upon death)
var globalNetNRG=0;
var time=0;
var redResource=0;
var blueResource=0;
...
var aveChilds=0; //Ratio of children to all petris
...
0.29:
29/2/17: Best score = how long petri has been alive for... Possibly implement score which stacks when parent dies (this.score = par.score+this.score)? Growth rate could get out of hand...
1.08:
23/3/17:
...
function cycle() {
  ...
  aveChilds=netChilds/(LIVEPOP+deadanimals.length);
  aveLifespan=netAge/(LIVEPOP+deadanimals.length);
  aveNetNRG=globalNetNRG/(LIVEPOP+deadanimals.length);
  if(time%100 == 0){
    aveChildsHist.push(aveChilds);
    childsPerParentHist.push(childsPerParent);
    aveLifespanHist.push(aveLifespan);
    aveNetNRGHist.push(aveNetNRG);
    redResourceHist.push(redResource);
    blueResourceHist.push(blueResource);
    greenResourceHist.push(greenResource);

    if(aveChilds>maxAveChilds){
      maxAveChilds=aveChilds;
    }
    ...
    if(aveNetNRG>maxAveNetNRG){
      maxAveNetNRG=aveNetNRG;
    }
...
var console = {
	setup : function() {
		...
		var posy=15;
		ctx3.fillText("PETRI 1.11", 10, posy);
    ...
    if(backpropMode==0) {
      ctx3.fillText("BACKPROP OFF",posx2+5,posy+=20);
    } else if(backpropMode==1){
      ctx3.fillText("BACKPROP PARENT",posx2+5,posy+=20);
    } else {
      ctx3.fillText("BACKPROP LINEAGE",posx2+5,posy+=20);
    }
    if(forthpropMode==0) {
      ctx3.fillText("FORTHPROP OFF",posx2+5,posy+=20);
    } else if(forthpropMode==1){
      ctx3.fillText("FORTHPROP PARENT",posx2+5,posy+=20);
    } else {
      ctx3.fillText("FORTHPROP LINEAGE",posx2+5,posy+=20);
    }
	},
	update : function() {
      ...
      ctx4.beginPath();
      var lineX=(CONSOLEX/(aveChildsHist.length-1)); //when aveChildsHist.length==2, there are two points. Then lineX==CONSOLEX.
      ...
      var endY=0;
      var show=0;
      for(var i=0; i<aveChildsHist.length-1; i++){ // Will begin once array has length of at least 2. Initial length==1. Wait until length 2
        endY= 100*aveChildsHist[i+1]/maxAveChilds;//Height of line
        ...
        if(consoleMouse && abs(mouseX-posx)<10 && abs(mouseY-(posy-initY))<10){
          if(show==0) {
            ctx4.fillText("AVECHILDS: "+round(100*aveChildsHist[i])/100, 10, posy-90);
      ...
      if(show==0){
        ctx4.fillText("AVECHILDS: "+round(100*aveChilds)/100, 10, posy-90);
      }
      ...
      endY=0;
      show=0;
      for(var i=0; i<aveNetNRGHist.length-1; i++){ // Will begin once array has length of at least 2. Initial length==1. Wait until length 2
        endY= round(100*aveNetNRGHist[i+1]/maxAveNetNRG);//Height of line
        ctx4.moveTo(posx,posy-initY); // First point will be on (0, 500)
        ctx4.lineTo(posx+lineX, posy-endY);

        if(consoleMouse && abs(mouseX-posx)<10 && abs(mouseY-(posy-initY))<10){
          if(show==0) {
            ctx4.fillText("AVENETNRG: "+round(aveNetNRGHist[i]), 10, posy-90);
            ctx4.fillText("TIME: "+(i*100), 10, posy-80);
            show=1;
          }
        }
      ...
      if(show==0){
        ctx4.fillText("AVENETNRG: "+round(aveNetNRG), 10, posy-90);
      }
      ctx4.stroke();
      ...
      for(var i=0; i<redResourceHist.length-1; i++){ // Will begin once array has length of at least 2. Initial length==1. Wait until length 2
        ...
        if(consoleMouse && abs(mouseX-posx)<10 && abs(mouseY-(posy-initY))<10){
          if(show==0) {
            ctx4.fillText("RRESO: "+round(redResourceHist[i]), 10, posy-90);
            show=1;
          }
          if(showTime==0){
            ctx4.fillText("TIME: "+(i*100), 10, posy-60);
            showTime=1;
          }
        }
        posx+=lineX;
      ...
      for(var i=0; i<greenResourceHist.length-1; i++){ // Will begin once array has length of at least 2. Initial length==1. Wait until length 2
        ...
        if(consoleMouse && abs(mouseX-posx)<10 && abs(mouseY-(posy-initY))<10){
          if(show==0) {
            ctx4.fillText("GRESO: "+round(greenResourceHist[i]), 10, posy-80);
            show=1;
          }
          if(showTime==0){
            ctx4.fillText("TIME: "+(i*100), 10, posy-60);
      ...
      for(var i=0; i<blueResourceHist.length-1; i++){ // Will begin once array has length of at least 2. Initial length==1. Wait until length 2
        ...
        if(consoleMouse && abs(mouseX-posx)<10 && abs(mouseY-(posy-initY))<10){
          if(show==0) {
            ctx4.fillText("BRESO: "+round(blueResourceHist[i]), 10, posy-70);
            show=1;
          }
          if(showTime==0){
            ctx4.fillText("TIME: "+(i*100), 10, posy-60);
...
/*
0.21:
7/2/17: Created name function to help keep track of descendant offspring.
*/
function namer() {
	var n="";
	for(var i=0;i<4;i++) {
...
/*
0.29:
29/2/17: Implementing gradient descent via a "back-propagation" is POSSIBLE!! Not going to be using a scientifically
accurate definition- just my own experimental attempt at creating a recurrent mutation tweaker that follows a similar idea.
...
4. For now, only children deaths influence parent LRs. Parents have no influence on children LRs upon death. Success rate/
	...
	TLDR: natural selection happens, obviously.
*/
...
17/3/17: No more I can do on Javascript; too slow- need to proceed from using linear values to multiplicative or softsign function, if possible.
  Time to humble myself and write in c++. Furthermore: If our inputs are telling us the positivity/negativity and extremity of our inputs,
  shouldnt we simply manipulate our inputs rather than generating new numbers at each layer?
1.07:
22/3/17: Possibly move range into float values??
1.08:
22/3/17: Switching to multiplication. Keep biases, w1, w2. DONT USE NEGATIVES???
*/
function Neuron() {
  ...
  this.minOut=-0.5;
	...
	for(var i=0;i<BRAINSIZECAP;i++) {
		this.bias[i]=0;
		var c=round(500*randn_bm())/1000; //Half range of norm. dist.
		this.weights[0][i]=c;
    this.weights[1][i]=c;
		this.cost+=abs(c);
		this.cost+=abs(c);
  ...
  this.outWeight=function(idx) { // if this.is>=0 return this.is*weights[0]
    return this.bias[idx]+this.softSign(this.is)*this.weights[(this.is>=0 ? 0:1)][idx];
  }

  this.softSign=function(is) {
...
29/2/17: Idea: Allow mouth(s), eyes to detect similarity in names of other petris:
	(AAAA <-> AAAA) = 1
	(AAAA <-> AAAB) = 0.8
	(AAAA <-> AABB) = 0.4
	(AAAA <-> ABBB) = -0.2
...
*/
	this.decay=function() {
		if(this.energy<=0) {
			...
			if(this.pidx!=null) { //petri needs to tell parent/children its dead... needs to know if parent is alive. If parent dead, pidx will be negative (-(pidx+1)).
        ...
        if(backpropMode!=0){
          ...
          if(backpropMode==1){
            if(this.pidx>=0) {
              ...
              this.backdescprop(this.pidx, scale);
            }
          }else if(backpropMode==2){
            var ancestor = this.pidx;
            while(ancestor!=null) {
              if(ancestor>=0){ //if ancestor is alive
                //this.backprop(ancestor, scale);
                this.backdescprop(ancestor, scale);
		...
		if(this.outputs[4]>=0.33) {
			if(this.size>=2*this.minSize) {
				if(LIVEPOP<POPCAP) {
					var i=0;
					while (animals[i]!=null) {
						if(animals[i].top<SCORESCAP || animals[i].alive) {  //Alive clause effectively determines that petri isnt overwriting itself
							i++;
						} else {
							break;
						}
          ...
          if(scoreType==0){
            ...
            if(forthpropMode==1) {
              if(this.pidx!=null && this.pidx>=0) {
                //this.forthprop(this.pidx); // mutate towards success
                animals[this.pidx].descendants++;
                this.descprop(this.pidx);
              } else if(this.pidx!=null){
                deadanimals[-(this.pidx+1)].descendants++;
              }
              ...
              if(ancestor!=null) {
                if(ancestor>=0) {
                  //this.forthprop(ancestor); // mutate towards success
                  animals[ancestor].descendants++;
                  this.descprop(ancestor);
                  ancestor=animals[ancestor].pidx;
                } else {
  ...
  this.mutate=function(a) {
    ...
    a.velres=this.velres;
    a.rotres=this.rotres;
    a.senmuts[0]=this.senmuts[0];
    a.senmuts[1]=this.senmuts[1];
    a.velres+=this.senmuts[0]/this.descendants; // child attributes displaced incrementally...
    a.rotres+=this.senmuts[1]/this.descendants;
    ...
    a.maxSize=this.maxSize;
    a.minSize=this.minSize;
    a.maxSize+=round(this.maxszmut/this.descendants);
    ...
    a.minSize+=round(this.minszmut/this.descendants);
    ...
    a.mouth.mutrs[0]=this.mouth.mutrs[0];
    a.mouth.mutrs[1]=this.mouth.mutrs[1];

    a.mouth.dissen+=this.mouth.mutrs[0]/this.descendants;
    a.mouth.dissen=round(1000*a.mouth.dissen)/1000;
    a.mouth.dirsen+=this.mouth.mutrs[1]/this.descendants;
    a.mouth.dirsen=round(1000*a.mouth.dirsen)/1000;

    this.mouth.mutrs[0]+=(round(Math.random()*2)-1)/1000;
    this.mouth.mutrs[0]=round(1000*this.mouth.mutrs[0])/1000;
    this.mouth.mutrs[1]+=(round(Math.random()*2)-1)/1000;
    ...
    for(var i=0, bL=BRAINLAYERSCAP; i<bL; i++) {
      for(var j=0, bS=BRAINSIZECAP; j<bS; j++) {
        ...
        for(var k=0, bS2=BRAINSIZECAP; k<bS2; k++) {
          ...
          a.brain[i][j].weights[0][k]+=this.brain[i][j].mutrw1[k]/this.descendants; //Child attributes mutated randomly by parent genes
          a.brain[i][j].weights[1][k]+=this.brain[i][j].mutrw2[k]/this.descendants;
          a.brain[i][j].bias[k]+=this.brain[i][j].mutrb[k]/this.descendants;

          a.brain[i][j].weights[0][k]=round(1000*a.brain[i][j].weights[0][k])/1000;
          a.brain[i][j].weights[1][k]=round(1000*a.brain[i][j].weights[1][k])/1000;
          a.brain[i][j].bias[k]=round(1000*this.brain[i][j].mutrb[k])/1000;

          this.brain[i][j].mutrw1[k]+=(round(Math.random()*2)-1)/1000;
          this.brain[i][j].mutrw1[k]=round(1000*this.brain[i][j].mutrw1[k])/1000;
          this.brain[i][j].mutrw2[k]+=(round(Math.random()*2)-1)/1000;
          this.brain[i][j].mutrw2[k]=round(1000*this.brain[i][j].mutrw2[k])/1000;
          this.brain[i][j].mutrb[k]+=(round(Math.random()*2)-1)/1000;
          this.brain[i][j].mutrb[k]=round(1000*this.brain[i][j].mutrb[k])/1000;
    ...
    for(var i=0, eC=EYECAP; i<eC; i++) {
      if(i>=a.eyeNumber) {
        ...
        a.eyes[i].mutrs[0]=this.eyes[i].mutrs[0];
        a.eyes[i].mutrs[1]=this.eyes[i].mutrs[1];

        a.eyes[i].dissen+=this.eyes[i].mutrs[0]/this.descendants; //Child attributes displaced by parent muts...
        a.eyes[i].dissen=round(1000*a.eyes[i].dissen)/1000;
        a.eyes[i].dirsen+=this.eyes[i].mutrs[1]/this.descendants;
        a.eyes[i].dirsen=round(1000*a.eyes[i].dirsen)/1000;

        this.eyes[i].mutrs[0]+=(round(Math.random()*2)-1)/1000; //Parent genes displaced...
        this.eyes[i].mutrs[0]=round(1000*this.eyes[i].mutrs[0])/1000;
        this.eyes[i].mutrs[1]+=(round(Math.random()*2)-1)/1000;
  ...
  this.highlight=function() {
    ...
    if(terrainMouse && mouseX>=this.x-50 && mouseX<this.x+50 && mouseY>=this.y-50 && mouseY<this.y+50) {
      ...
      ctx4.fillText("TOP: "+round(this.top),posx, posy+=10);
  		ctx4.fillText("B$: "+(round(100*this.brainCost)/100),posx,posy+=10);
  		posy+=10;
      ...
      var neuLay=null;
  		...
  		*/
          ...
          ctx4.fillText("B$: "+round(neu.cost*10)/10,20,posy2+=10);
  				ctx4.fillText("W1",20,posy2+=10);
  				ctx4.fillText("W2",50,posy2);
  				ctx4.fillText("B",110,posy2);
  				for(var i=0, bS2=BRAINSIZECAP; i<bS2; i++) {
  					ctx4.fillText(neu.weights[0][i], 20, posy2+=10);
  					ctx4.fillText(neu.weights[1][i], 50, posy2);
  					ctx4.fillText(neu.bias[i], 110, posy2);
  				}
  			} else {
  				var posy2=30;
  				ctx4.fillStyle="#FFFFFF";
  				...
  				for(var i=0, bS2=BRAINSIZECAP; i<bS2; i++) {
  					...
  					ctx4.fillText(neu.mutrw2[i], 50, posy2);
  					ctx4.fillText(neu.mutrb[i], 110, posy2);
  				}
  			}
  		}
  ...
  this.backdescprop=function(index, scale) { // cI= child index, pI= parent index (if parent is alive)
    pI = animals[index];
    for(var i=0, bL=BRAINLAYERSCAP; i<bL; i++) {
      for(var j=0, bS=BRAINSIZECAP; j<bS; j++) {
        for(var k=0, bS2=BRAINSIZECAP; k<bS2; k++) {
...
*/
  ...
  */
  this.descprop=function(index) { // cI= child index, pI= parent index (if parent is alive)
    var pI=animals[index];
    for(var i=0, bL=BRAINLAYERSCAP; i<bL; i++) {
      for(var j=0, bS=BRAINSIZECAP; j<bS; j++) {
        for(var k=0, bS2=BRAINSIZECAP; k<bS2; k++) {
